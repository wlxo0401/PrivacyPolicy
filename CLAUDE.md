...
1. **앱 코드 분석 먼저.** 해당 iOS 프로젝트 폴더(예: `/Users/kimjitae/Desktop/Project/Kimjitae/{AppName}/`)에서 실제 사용 SDK 확인:
   ...
   - **UMP(동의 관리) 확인**: `import UserMessagingPlatform` / `ConsentManager`·`LaunchConsentFlow` 존재 여부. 쓰면 정책의 처리 목적·위탁(AdMob/UMP)·권리 행사(동의 변경)에 반드시 반영. (LottoMaster·MoneyCalc·NumberOfPictures는 처음에 누락됐던 항목)
   - **Crashlytics 실사용 확인**: `import FirebaseCrashlytics`만으로 단정 금지. `AppDelegate`의 `setCrashlyticsCollectionEnabled` 실값을 확인 — 보통 `#if DEBUG false / #else true`라 **출시 빌드에서만 수집**. 쓰면 "Firebase Crashlytics 수집(통상 90일, 디버그 비수집)"으로, 안 쓰면 "Apple 시스템 크래시(옵트인)"로 정확히 구분. (앱마다 사용/미사용/오선언이 제각각 — HashTag는 Crashlytics 쓰는데 "Apple 시스템 레벨"로 오선언돼 있었음)
